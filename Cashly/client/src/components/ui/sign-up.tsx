...
import { Gem, PartyPopper, Loader, ArrowRight, Mail, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle } from "lucide-react";
...
export const glassButtonVariants = cva("relative isolate all-unset cursor-pointer rounded-full transition-all", { variants: { size: { default: "text-base font-medium", sm: "text-sm font-medium", lg: "text-lg font-medium", icon: "h-10 w-10" } }, defaultVariants: { size: "default" } });
...
// --- CHILD COMPONENTS ---
const getModalSteps = (mode: 'login' | 'signup') => [
    { message: mode === 'login' ? "Verifying..." : "Creating Account...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: mode === 'login' ? "Logging in..." : "Onboarding you...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: mode === 'login' ? "Almost there..." : "Finalizing...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: mode === 'login' ? "Welcome Back!" : "Welcome Aboard!", icon: <PartyPopper className="w-12 h-12 text-green-500" /> }
];
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Cashly", mode = 'signup', onSubmit, onComplete }: AuthComponentProps) => {
    ...
    const [authStep, setAuthStep] = useState<"name" | "email" | "password" | "confirmPassword">(mode === 'signup' ? "name" : "email");
    const [modalStatus, setModalStatus] = useState<'closed' | 'loading' | 'error' | 'success'>('closed');
    const [modalErrorMessage, setModalErrorMessage] = useState('');
    const confettiRef = useRef<ConfettiRef>(null);

    // We check mode to determine steps
    ...
    const handleFinalSubmit = async (e: React.FormEvent) => {
        ...
        // For login, we might trigger early if password step is done, but let's keep consistent flow for now
        if (mode === 'login' && authStep !== 'password') return;
        if (mode === 'signup' && (modalStatus !== 'closed' || authStep !== 'confirmPassword')) return;
        ...
        // Simulate steps or validation
        // In a real app, you might want to call onSubmit immediately or after some "fake" steps
        // Let's do the API call first or concurrently with the animation
        ...
        try {
            ...
            // Wait for a moment to show the success state/confetti then complete
            setTimeout(() => {
                if (onComplete) onComplete();
            }, 2000); // 2 seconds to enjoy the confetti
            ...
            if (error.response) {
                ...
                if (serverMsg) {
                    ...
                    if (serverMsg.toLowerCase().includes('already exists') || serverMsg.toLowerCase().includes('duplicate')) {
                        errorMsg = mode === 'signup' ? "This email is already registered. Please login instead." : serverMsg;
                    } else if (serverMsg.toLowerCase().includes('invalid') && serverMsg.toLowerCase().includes('credentials')) {
                        errorMsg = "Invalid email or password. Please try again.";
                    } else if (serverMsg.toLowerCase().includes('not found') || serverMsg.toLowerCase().includes('no user')) {
    ...
    const handleGoBack = () => {
        ...
        else if (authStep === 'email' && mode === 'signup') setAuthStep('name');
    };
    ...
    return (
        ...
        `}</style>
            ...
            {/* Show modal for all modes (login and signup) */}
            <>
            ...
            <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden")}>
                ...
                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                    <AnimatePresence mode="wait">
                        ...
                        {authStep === "name" && mode === 'signup' && <motion.div key="name-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                            <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-5xl tracking-tight text-foreground whitespace-nowrap">Let's start</p></div></BlurFade>
                            <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">What should we call you?</p></BlurFade>
                        </motion.div>}
                        ...
                        {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                            <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-5xl tracking-tight text-foreground whitespace-nowrap">
                                {mode === 'login' ? 'Welcome Back' : `Hi, ${name}`}
                            </p></div></BlurFade>
                            <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">{mode === 'login' ? 'Sign in to continue' : 'What is your email address?'}</p></BlurFade>
                        </motion.div>}
                        ...
                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">
                                {mode === 'login' ? 'Enter Password' : 'Create your password'}
                            </p></div></BlurFade>
                            {mode === 'signup' && <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">Your password must be at least 6 characters long.</p></BlurFade>}
                        </motion.div>}

                        {/* CONFIRM STEP (SIGNUP ONLY) */}
                        {authStep === "confirmPassword" && mode === 'signup' && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                    ...
                    <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        <AnimatePresence>
                            {authStep !== 'confirmPassword' && <motion.div key="fields-container" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                                <AnimatePresence>
                                    {authStep === 'name' && mode === 'signup' && <BlurFade key="name-field" inView={true} className="w-full">
                                        <div className="relative w-full">
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                <span className="glass-input-text-area"></span>
                                                <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2"><span className="text-lg">👋</span></div>
                                                <input ref={nameInputRef} autoFocus type="text" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={handleKeyDown} className={cn("relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none transition-[padding-right] duration-300 ease-in-out", isNameValid ? "pr-2" : "pr-0")} />
                                                <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isNameValid ? "w-10 pr-1" : "w-0")}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Next" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                            </div></div>
                                        </div>
                                    </BlurFade>}
                                </AnimatePresence>
                                ...
                                <AnimatePresence>
                                    {authStep === 'email' && <BlurFade delay={0} inView={true} className="w-full">
                                        <div className="relative w-full">
                                            <AnimatePresence>
                                                {email.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Email</label></motion.div>}
                                            </AnimatePresence>
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                <span className="glass-input-text-area"></span>
                                            ...
                                            {mode === 'signup' && <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>}
                                        </div>
                                    </BlurFade>}
                                </AnimatePresence>
                                <AnimatePresence>
                                    {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                        <div className="relative w-full">
                                            ...
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                ...
                                                <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", (isPasswordValid || mode === 'login') ? "w-10 pr-1" : "w-0")}>
                                                    {mode === 'login' ? (
                                                        <GlassButton type="submit" size="icon" aria-label="Login" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton>
                                                    ) : (
                        ...
                        <AnimatePresence>
                            {authStep === 'confirmPassword' && mode === 'signup' && <BlurFade key="confirm-password-field" className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
